...
def validate_source(base_url: str, sample_url: str | None = None) -> ValidationResult:
    ...
    try:
        ...
        if rr.status_code in (401, 403, 429):
            return ValidationResult("blocked", f"robots blocked ({rr.status_code})")
    except Exception as e:
        return ValidationResult("degraded", f"robots check failed: {e}")
    ...
    if sample_url:
        try:
            ...
            if r.status_code in (401, 403, 429) or "captcha" in body:
                return ValidationResult("blocked", f"sample blocked ({r.status_code})")
            if r.status_code >= 500:
                return ValidationResult("degraded", f"sample server error ({r.status_code})")
        except Exception as e:
            return ValidationResult("degraded", f"sample check failed: {e}")

    return ValidationResult("healthy", "validation ok")
