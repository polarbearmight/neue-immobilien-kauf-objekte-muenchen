...
SEARCH_URL = "https://planethome.de/"
...
def collect_planethome_listings() -> list[dict]:
    c = SafeCollector()
    c.assert_allowed("https://planethome.de/robots.txt", "/")
    try:
    ...
    print(f"INFO planethome parser: anchors={len(anchors)} rows={len(rows)}")
    return rows
