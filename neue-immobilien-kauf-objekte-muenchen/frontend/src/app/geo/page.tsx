...
export default function GeoPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold tracking-tight">Geo Deal Heatmap</h1>

      <div className="grid gap-3 rounded-xl border p-3 md:grid-cols-6 text-sm">
        <div><label className="mb-1 block text-muted-foreground">Mode</label><select className="w-full rounded border px-2 py-1" value={mode} onChange={(e) => setMode(e.target.value)}><option value="median_price">Median Price</option><option value="deal_density">Deal Density</option><option value="new_density">New Listings Density</option><option value="off_market">Off-Market Density</option><option value="price_drop">Price Drop Density</option><option value="yield">Investment Yield Potential</option></select></div>
        ...
        <div><label className="mb-1 block text-muted-foreground">Score threshold</label><input type="number" min={0} max={100} className="w-full rounded border px-2 py-1" value={minScore} onChange={(e) => setMinScore(Number(e.target.value || 0))} /></div>
        ...
        <div><label className="mb-1 block text-muted-foreground">View</label><div className="flex gap-2"><button className={`rounded border px-2 py-1 ${view==="district"?"bg-muted":""}`} onClick={() => setView("district")}>district</button><button className={`rounded border px-2 py-1 ${view==="coordinate"?"bg-muted":""}`} onClick={() => setView("coordinate")}>coordinate</button></div></div>
      </div>

      <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
        <div className="rounded-xl border p-3">
          <h2 className="mb-2 text-sm font-medium">{view === "district" ? "District Intelligence Grid" : "Coordinate Cells"}</h2>
          {view === "district" ? (
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {sortedRows.map((r) => (
                <div key={r.district} className="rounded border p-2 text-xs">
                  ...
                  <p>Avg score: {r.average_deal_score ? Math.round(r.average_deal_score) : "-"}</p>
        ...
        <div className="rounded-xl border p-3 text-sm">
          ...
          <div className="mt-3 rounded border p-2 text-xs text-muted-foreground">
            <p>Total listings: {summary?.total_listings ?? 0}</p>
            <p>Districts: {summary?.districts ?? 0}</p>
      ...
      <div className="rounded-xl border p-3">
        <h2 className="mb-2 text-sm font-medium">District Ranking Table</h2>
