...
export default function DashboardPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        const [lRes, sRes, aRes] = await Promise.all([
          fetch(`${API_URL}/api/listings?${params.toString()}`, { cache: "no-store", signal: controller.signal, headers: authHeaders() }),
          fetch(`${API_URL}/api/stats?days=7`, { cache: "no-store", signal: controller.signal, headers: authHeaders() }),
          fetch(`${API_URL}/api/analytics?days=30`, { cache: "no-store", signal: controller.signal, headers: authHeaders() }),
        ]);
  ...
  useEffect(() => {
    ...
    const loadDistricts = async () => {
      try {
        const res = await fetch(`${API_URL}/api/districts`, { cache: "no-store", signal: controller.signal, headers: authHeaders() });
        if (!res.ok) return;
        const data = await res.json();
      ...
      {loading ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 animate-pulse rounded-2xl border border-border bg-muted/40" />)}</div> : (
        <>
          <div className="grid gap-3 md:hidden">
            {mobileKpiMode === "market" ? <><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Neue Listings</CardTitle></CardHeader><CardContent className="text-2xl font-semibold dark:text-amber-50">{stats?.new_listings ?? 0}</CardContent></Card><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Ø Preis / m²</CardTitle></CardHeader><CardContent className="text-2xl font-semibold dark:text-amber-50">{eur(stats?.avg_price_per_sqm)}</CardContent></Card></> : mobileKpiMode === "deals" ? <><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Top-Deals</CardTitle></CardHeader><CardContent className="text-2xl font-semibold dark:text-amber-50">{stats?.top_deals ?? 0}</CardContent></Card><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Treffer aktuell</CardTitle></CardHeader><CardContent className="text-2xl font-semibold dark:text-amber-50">{filtered.length}</CardContent></Card></> : <><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Aktive Quellen</CardTitle></CardHeader><CardContent className="text-2xl font-semibold dark:text-amber-50">{sources.length - 1}</CardContent></Card><Card className="rounded-[1.6rem] border-border/80 bg-card/95 dark:border-amber-400/16 dark:bg-[linear-gradient(180deg,rgba(34,27,14,0.9),rgba(10,12,16,0.98))] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]"><CardHeader><CardTitle className="text-sm dark:text-amber-100/78">Status</CardTitle></CardHeader><CardContent className="text-base font-semibold capitalize dark:text-amber-50">{scan?.status || "idle"}</CardContent></Card></>}
          </div>
