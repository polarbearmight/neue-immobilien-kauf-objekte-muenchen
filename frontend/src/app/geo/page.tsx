...
type DistrictRow = { district: string; listing_count: number; median_price_per_sqm: number | null; average_price_per_sqm: number | null; average_deal_score: number | null; average_off_market_score: number | null; top_deal_count: number; just_listed_count: number; price_drop_count: number; hotspot_score: number; trend: "up" | "down" | "flat" | string };
...
export default function GeoPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setRows(d?.rows || []); setHotspots(h?.rows || []); setSummary(s || null); setCells(c?.rows || []);
    });
  ...
  return (
    <RoleGuard minRole="pro">
      <div className="space-y-4">
        <RoleBanner role="pro" title="Premium area" body="Geo-Heatmap, Hotspots und Koordinatenzellen sind für Pro und Admin sichtbar." />
        <h1 className="text-2xl font-semibold tracking-tight">Geo-Heatmap</h1>
        <div className="grid gap-3 rounded-xl border p-3 text-sm md:grid-cols-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
          <div className="rounded-xl border p-3">
            ...
            {view === "district" ? <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{sortedRows.map((r) => <div key={r.district} className="rounded border p-2 text-xs"><p className="font-semibold">{r.district}</p><p>Median: {eur(r.median_price_per_sqm)}</p><p>Listings: {r.listing_count}</p><p>Top-Deals: {r.top_deal_count}</p><p>Ø Score: {r.average_deal_score ? Math.round(r.average_deal_score) : "-"}</p><p>Trend: {r.trend === "down" ? "↓" : r.trend === "up" ? "↑" : "→"}</p></div>)}</div> : <div className="space-y-1 text-xs">{cells.slice(0, 40).map((c) => <p key={c.cell}>{c.cell} · listings {c.listing_count} · top deals {c.top_deal_count}</p>)}</div>}
          </div>
          <div className="rounded-xl border p-3 text-sm"><h2 className="mb-2 font-medium">Top-Hotspots</h2><ol className="space-y-1 text-xs">{hotspots.slice(0, 5).map((h, idx) => <li key={h.district}>{idx + 1}. {h.district} (score {Math.round(h.hotspot_score)})</li>)}</ol><div className="mt-3 rounded border p-2 text-xs text-muted-foreground"><p>Listings gesamt: {summary?.total_listings ?? 0}</p><p>Stadtteile: {summary?.districts ?? 0}</p><p>Top-Deals: {summary?.top_deals ?? 0}</p></div></div>
        </div>
        <div className="rounded-xl border p-3"><h2 className="mb-2 text-sm font-medium">Stadtteil-Ranking</h2><div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b text-left text-muted-foreground"><th className="px-2 py-1">district</th><th className="px-2 py-1">listings</th><th className="px-2 py-1">median €/m²</th><th className="px-2 py-1">avg deal score</th><th className="px-2 py-1">top deals</th><th className="px-2 py-1">off-market</th><th className="px-2 py-1">just listed</th><th className="px-2 py-1">hotspot score</th></tr></thead><tbody>{sortedRows.map((r) => <tr key={r.district} className="border-b"><td className="px-2 py-1">{r.district}</td><td className="px-2 py-1">{r.listing_count}</td><td className="px-2 py-1">{eur(r.median_price_per_sqm)}</td><td className="px-2 py-1">{r.average_deal_score ? Math.round(r.average_deal_score) : "-"}</td><td className="px-2 py-1">{r.top_deal_count}</td><td className="px-2 py-1">{r.average_off_market_score ? Math.round(r.average_off_market_score) : 0}</td><td className="px-2 py-1">{r.just_listed_count}</td><td className="px-2 py-1">{Math.round(r.hotspot_score)}</td></tr>)}</tbody></table></div></div>
      </div>
    </RoleGuard>
  );
}
