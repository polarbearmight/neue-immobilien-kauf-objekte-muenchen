...
function summarizeExplain(raw?: string | null) {
  ...
  try {
    ...
    return [`Cluster ${data.cluster_size ?? "-"}`, `Freshness ${Math.round(data.freshness_hours ?? 0)}h`, `Penalty ${Math.round(data.suspicious_penalty ?? 0)}`, `Final ${Math.round(data.final ?? 0)}`].join(" · ");
  } catch {
    return raw;
  }
}
  ...
  return (
    <RoleGuard minRole="pro">
      <div className="space-y-4">
        <RoleBanner role="pro" title="Premium area" body="Off-Market ist für Pro und Admin sichtbar. Admin sieht zusätzlich alle Admin-Bereiche." />
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-end gap-3 text-sm">
            ...
            <div>
              ...
              <datalist id="offmarket-districts">{knownDistricts.map((d) => <option key={d} value={d} />)}</datalist>
            </div>
          </div>
        </div>
            ...
            {rows.map((l) => {
              ...
              return (
                <div key={`${l.source}-${l.source_listing_id}`} className="rounded-xl border p-4 text-sm">
                  ...
                  <div className="mt-2 flex gap-2"><a href={l.url} target="_blank" rel="noreferrer" className="rounded border px-2 py-1 text-xs">Open</a></div>
                </div>
              );
            })}
          </div>
